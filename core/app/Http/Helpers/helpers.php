...
function send_invoice($invoice, $type, $shortcodes = [])
{
    $general = GeneralSetting::first();
    $email_template = \App\EmailTemplate::where('act', $type)->where('email_status', 1)->first();
    if ($general->en != 1 || !$email_template) {
    ...
    $message = shortcode_replacer("{{name}}", $invoice->name, $general->etemp);
    $message = shortcode_replacer("{{message}}", $email_template->email_body, $message);
    ...
    if ($config->name == 'php') {
        send_php_mail($invoice->email, $invoice->name, $general->efrom, $email_template->subj, $message);
    } else if ($config->name == 'smtp') {
        send_smtp_mail($config, $invoice->email, $invoice->name, $general->efrom, $general->sitetitle, $email_template->subj, $message);
    } else if ($config->name == 'sendgrid') {
        send_sendgrid_mail($config, $invoice->email, $invoice->name, $general->efrom, $general->sitetitle, $email_template->subj, $message);
    } else if ($config->name == 'mailjet') {
        send_mailjet_mail($config, $invoice->email, $invoice->name, $general->efrom, $general->sitetitle, $email_template->subj, $message);
    }
}
